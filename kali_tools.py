...
# Session cache for _is_wsl_available() — avoids re-spawning `wsl` on every
# is_installed() call. None = not yet probed.
...
def _is_wsl_available() -> bool:
    ...
    Cached for the process lifetime: is_installed() calls this per tool, so a
    single /tools listing (20 tools) could otherwise spawn ~20 `wsl` probe
    processes on Windows — slow. WSL availability is stable within a session.
    Set _WSL_AVAILABLE_CACHE back to None to force a re-probe if ever needed."""
