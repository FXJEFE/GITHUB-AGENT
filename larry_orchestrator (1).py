...
def _find_launchers() -> Path:
    # 0) Explicit override for exotic layouts (deep nesting, junctions, CI):
    #    LARRY_LAUNCHERS points at launchers/, or LARRY_ROOT at the repo root.
    for var in ("LARRY_LAUNCHERS", "LARRY_ROOT"):
        env = os.environ.get(var)
        if not env:
            continue
        p = Path(env).expanduser().resolve()
        if (p / "start_fullstack.py").exists():
            return p
        if (p / "launchers" / "start_fullstack.py").exists():
            return p / "launchers"
        print(f"[orchestrator] {var}={env} set but start_fullstack.py not "
              f"found there — falling back to auto-detect.", flush=True)
    here = Path(__file__).resolve()          # resolve() also follows symlinks
    # 1) Anchor on start_fullstack.py (original behaviour).
    for base in (here.parent, *here.parents):
        if (base / "start_fullstack.py").exists():
            return base
        if (base / "launchers" / "start_fullstack.py").exists():
            return base / "launchers"
    # 2) Project-marker fallback: larry_config.json marks the repo root even
    #    when start_fullstack.py is absent (orchestrator then runs on its
    #    built-in helper fallbacks instead of crashing).
    for base in (here.parent, *here.parents):
        if (base / "larry_config.json").exists():
            lp = base / "launchers"
            return lp if lp.is_dir() else base
    # 3) Last resort: assume sibling layout (original behaviour).
    return here.parent
...
# (section, key) -> expected type. SCOPE: only keys THIS file reads — this is
# not a schema for all of larry_config.json. Warn-only; never blocks startup.
_CONFIG_SPEC: dict[tuple[str, str], type] = {
    ("dashboard", "host"): str,
    ("dashboard", "port"): int,
    ("ollama", "host"): str,
    ("ollama", "keep_alive"): str,
    ("ollama", "num_parallel"): int,
    ("ollama", "max_loaded_models"): int,
    ("features", "mcp_enabled"): bool,
    ("mcp", "config_path"): str,
    ("orchestrator", "startup_delay_seconds"): int,
    ("orchestrator", "warm_fast_model"): bool,
    ("orchestrator", "start_governor"): bool,
    ("orchestrator", "start_api"): bool,
    ("orchestrator", "start_docker"): bool,
    ("orchestrator", "preload_mcp"): bool,
    ("orchestrator", "open_browser"): bool,
    ("orchestrator", "wsl_distro"): str,
    ("orchestrator", "start_agent_cli"): bool,
    ("orchestrator", "auto_restart"): bool,
    ("orchestrator", "restart_limit"): int,
    ("orchestrator", "require_fullstack"): bool,
    ("orchestrator", "agent_no_console"): bool,
}


def validate_config(cfg: dict) -> int:
    """Type-check the keys the orchestrator consumes. Returns warning count."""
    warns = 0
    for (section, key), expected in _CONFIG_SPEC.items():
        sec = cfg.get(section)
        if not isinstance(sec, dict) or key not in sec:
            continue                                   # absent = fine (defaults)
        val = sec[key]
        # bool is a subclass of int — don't accept true/false where int expected
        ok = isinstance(val, bool) if expected is bool else (
            isinstance(val, expected) and not isinstance(val, bool))
        if not ok:
            log(f"config: {section}.{key} = {val!r} — expected "
                f"{expected.__name__}, got {type(val).__name__}.", "WARN")
            warns += 1
    port = (cfg.get("dashboard", {}) or {}).get("port")
    if isinstance(port, int) and not isinstance(port, bool) and not (0 < port < 65536):
        log(f"config: dashboard.port = {port} — outside 1-65535.", "WARN")
        warns += 1
    return warns
...
def _spawn(name: str, argv: list[str], cwd: Path, logfile: str, hidden: bool = False,
           restartable: bool = False, stdin_devnull: bool = False) -> None:
    ...
    _children.append({"name": name, "proc": proc, "log": fh,
                      "argv": list(argv), "cwd": str(cwd), "flags": flags,
                      "stdin_devnull": stdin_devnull,
                      "restartable": restartable, "restarts": 0,
                      "exit_logged": False})
...
def start_wsl_keepalive(cfg: dict) -> None:
    ...
    argv = ["wsl", "-d", distro, "--", "sleep", "infinity"]
    proc = subprocess.Popen(argv, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                            creationflags=CREATE_NO_WINDOW)
    _children.append({"name": f"wsl-keepalive[{distro}]", "proc": proc, "log": None,
                      "argv": argv, "cwd": None, "flags": CREATE_NO_WINDOW,
                      "stdin_devnull": False,
                      "restartable": True, "restarts": 0, "exit_logged": False})
...
def start_fullstack(cfg: dict, py: str) -> bool:
    """Launch the Telegram-bot full stack. Returns True if it was launched (or
    is already handled), False if the launcher file is missing — the only
    failure --require-fullstack can meaningfully guard against here (a launched
    bot may still crash later; that surfaces via the supervision loop)."""
...
def start_agent_cli(cfg: dict, py: str, no_console: bool = False) -> None:
    ...
    no_console (recommendation #6, via --no-agent-console or
    orchestrator.agent_no_console): drop the separate console window. HONEST
    CAVEAT — verified against agent_v2.py: main() is a blocking input()-driven
    REPL whose `except EOFError` branch exits cleanly on end-of-stream. Without
    a real console its stdin is closed, so input() raises EOFError immediately
    and the agent shuts itself down within milliseconds. There is currently NO
    headless/daemon mode in agent_v2.py, so 'hidden + logging only' is not a
    working configuration. The truthful behaviour of this flag is therefore:
    DO NOT launch the interactive agent at all, and say why — the Telegram bot
    (start_fullstack) is the real non-interactive frontend. This flag becomes
    genuinely useful only once agent_v2.py grows a non-interactive mode."""
    if not orch_opt(cfg, "start_agent_cli", True):
        log("Interactive agent CLI disabled (orchestrator.start_agent_cli=false).")
        return
    if no_console or orch_opt(cfg, "agent_no_console", False):
        log("Agent CLI skipped: --no-agent-console set, but agent_v2.py is a "
            "blocking input() REPL that would exit on EOF with no console. Use "
            "the Telegram bot for headless access, or add a daemon mode to "
            "agent_v2.py.", "WARN")
        return
    ...
    try:
        ...
        _children.append({"name": "agent_cli", "proc": proc, "log": None,
                          "argv": [py, str(AGENT_CLI)], "cwd": str(SRC),
                          "flags": flags, "stdin_devnull": False,
                          "restartable": False, "restarts": 0, "exit_logged": False})
...
# ── supervise / teardown ─────────────────────────────────────────────────────
def _restart_child(child: dict, py_unused=None) -> bool:
    """Respawn a child from its recorded argv/cwd/flags. Returns True on success.
    Only ever called for children spawned with restartable=True."""
    argv = child.get("argv")
    if not argv:
        return False
    try:
        if child.get("log"):
            # _spawn-style child: reuse a fresh append to the same logfile object.
            fh = child["log"]
            try:
                fh.write(f"\n===== {child['name']} RESTARTED {datetime.now().isoformat()} =====\n")
                fh.flush()
            except Exception:
                fh = None
            proc = subprocess.Popen(argv, cwd=child.get("cwd") or None,
                                    stdout=(fh or subprocess.DEVNULL),
                                    stderr=subprocess.STDOUT,
                                    stdin=(subprocess.DEVNULL if child.get("stdin_devnull") else None),
                                    creationflags=child.get("flags", 0))
        else:
            # DEVNULL-style child (ollama serve, wsl keepalive).
            proc = subprocess.Popen(argv, cwd=child.get("cwd") or None,
                                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                                    creationflags=child.get("flags", 0))
        child["proc"] = proc
        child["restarts"] = child.get("restarts", 0) + 1
        child["exit_logged"] = False
        log(f"Restarted {child['name']} (PID {proc.pid}) "
            f"[restart #{child['restarts']}].")
        return True
    except Exception as e:
        log(f"Failed to restart {child['name']}: {e}", "WARN")
        return False


def supervise(cfg: dict) -> None:
    """Watch children until _stop or everything is gone.

    Over the original 'all exited -> break' behaviour this adds:
      • every exit is logged BY NAME the first time it's seen (incl. the WSL
        keepalive — folds recommendation #3's visibility into the one loop)
      • optional bounded auto-restart of children marked restartable
        (ollama serve, wsl keepalive), gated by orchestrator.auto_restart
        (default false) with orchestrator.restart_limit tries each (default 3).

    Auto-restart is deliberately OFF by default: silently resurrecting a
    mis-configured Ollama can hide the real fault. Turn it on only once the
    stack is stable."""
    auto = bool(orch_opt(cfg, "auto_restart", False))
    limit = int(orch_opt(cfg, "restart_limit", 3))
    if auto:
        log(f"Auto-restart ON for restartable children (limit {limit} each).")
    while not _stop:
        alive = 0
        for child in _children:
            if child["proc"].poll() is None:
                ...
                continue
            # child has exited
            if not child.get("exit_logged"):
                rc = child["proc"].returncode
                log(f"Child '{child['name']}' exited (code {rc}).", "WARN")
                child["exit_logged"] = True
            if auto and child.get("restartable") and child.get("restarts", 0) < limit:
                if _restart_child(child):
                    alive += 1
        if _children and alive == 0:
...
def main() -> int:
    ...
    parser.add_argument("--no-agent-console", action="store_true",
                        help="Launch the agent without its own console. NOTE: "
                             "agent_v2.py is a blocking input() REPL, so this "
                             "currently just skips it (see start_agent_cli).")
    parser.add_argument("--with-api", action="store_true")
    parser.add_argument("--with-docker", action="store_true")
    parser.add_argument("--require-fullstack", action="store_true",
                        help="Treat a missing/failed full-stack launcher as fatal: "
                             "tear down Tier 0 and exit non-zero instead of "
                             "continuing without the Telegram bot.")
    ...
    nwarn = validate_config(cfg)
    if nwarn:
        log(f"config: {nwarn} type warning(s) above — using values as-is "
            "(orchestrator never blocks on config).", "WARN")
    ...
    if not args.minimal:
        ...
        if not _stop:
            ...
            if not args.no_fullstack:
                fs_ok = start_fullstack(cfg, py)
                if not fs_ok and (args.require_fullstack
                                  or orch_opt(cfg, "require_fullstack", False)):
                    log("FATAL: --require-fullstack set but the full-stack "
                        "launcher is missing. Tearing down Tier 0 and exiting.",
                        "WARN")
                    teardown()
                    return 2
            if not args.no_agent:
                start_agent_cli(cfg, py, no_console=args.no_agent_console)
